...
//Regular jobs
	...
	//Prisoner (Orange)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='FF6600'><th colspan='[length(prisoner_positions)]'><a href='?src=\ref[src];jobban3=prisondept;jobban4=\ref[M]'>Prisoner Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in prisoner_positions)
			if(!jobPos)	continue
			var/datum/job/job = job_master.GetJob(jobPos)
			if(!job) continue
