...
/datum/game_mode/epidemic/send_intercept()
	...
	intercepttext += "<B>* A cure is to be researched immediately, but OmniWatch intellectual property must be respected. To prevent knowledge of [virus_name] from falling into unauthorized hands, all medical staff that work with the pathogen must be enhanced with a OmniWatch loyality implant.</B><BR>" // Prison Station TODO: Decide if this is even necessary and adjust this as needed.
